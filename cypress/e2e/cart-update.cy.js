...
describe("Delete items from Cart", { viewportHeight: 1200 }, () => {
  ...
  it.only("Delete items from cart", () => {
    ...
    InventoryPage.getShoppingCartBadge()
        .should('contain',ids.length)

    InventoryPage.getItemDescriptionBy(item.name)
        .find('#remove-sauce-labs-bolt-t-shirt')
        .should('have.text','Remove')
        .click()

    InventoryPage.getShoppingCartBadge()
        .should('contain',ids.length-1)
